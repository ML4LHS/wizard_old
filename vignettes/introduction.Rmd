...
temporal_file = system.file("../inst/extdata", "temporal_data.csv", package = "wisard")
fixed_file = system.file("../inst/extdata", "fixed_data.csv", package = "wisard")

temporal_data_1 =  data.table::fread(temporal_file) %>% 
  dplyr::mutate(time = time %>% lubridate::as_datetime())
# The fixed data should of type tibble or data.frame or disk frame.
...
temporal_data_2 = wisard::categorical_col_names_generator(temporal_data = temporal_data_1)
...
unique_columns = wisard::unique_variables(data = temporal_data_1)
unique_columns
```
...
temporal_data = wisard::date_to_time(temporal_data = temporal_data_1, fixed_data = fixed_data, units = period_measure$units)
```
